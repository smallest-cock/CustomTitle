...
void TitlesComponent::initHooks() {
	...
	hookWithCallerPost(Events::HUDBase_TA_DrawHUD, [this](ActorWrapper Caller, ...) {
		...
		if (gameHasRGBPresets) {
			...
			m_ingamePresets.forEachRGBPreset([this, rgbColor, userId](UGFxData_PRI_TA *pri, const TitleAppearance &preset) {
				if (!validUObject(pri))
					return;
	...
	hookWithCaller(Events::GFxData_PlayerTitles_TA_UpdateSelectedTitle, [this](ActorWrapper Caller, void *Params, ...) {
		if (!*m_enabled)
			return;
	...
	hookWithCallerPost(Events::GFxData_PlayerTitles_TA_UpdateSelectedTitle, [this](ActorWrapper Caller, void *Params, ...) {
		auto *caller = reinterpret_cast<UGFxData_PlayerTitles_TA *>(Caller.memory_address);
		if (!validUObject(caller))
			return;
	...
	*/
	...
	hookEventPost(Events::GameEvent_TA_Countdown_BeginState, [this](std::string event) { runCommand(Commands::broadcast, event); });
	hookEventPost(Events::GameEvent_Soccar_TA_Active_StartRound, [this](std::string event) { runCommand(Commands::broadcast, event); });
	hookEventPost(Events::GFxHUD_TA_HandleReplaceBot, [this](std::string event) { runCommand(Commands::broadcast, event); });

	hookEventPost(Events::GFxData_PRI_TA_SetPlayerTitle, [this](std::string) { refreshPriTitlePresets(); });
	hookEventPost(Events::EngineShare_X_EventPreLoadMap, [this](std::string) { m_ingamePresets.clear(); });
...
void TitlesComponent::initCvars() {
	...
	registerCommand(Commands::spawnCustomTitle, [this](...) {
		GAME_THREAD_EXECUTE({
			if (!*m_enabled) {
