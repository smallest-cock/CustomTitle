...
public:
	...
	void hookEvent(const std::string& eventName, HookType type, const std::function<void(std::string)>& callback)
	{
		...
		{
#ifdef THROW_ON_DOUBLE_HOOK // hardmode, for dev
		...
		case HookType::Post:
			...
			break;
		default:
			LOG("ERROR: Invalid hook type for event: \"{}\"", eventName);
			break;
		}
	...
	void unhookEvent(const std::string& eventName, HookType type)
	{
		...
		{
			LOG("ERROR: Unhooking function {} was unsuccessful (it was never hooked): \"{}\"",
			    type == HookType::Pre ? "Pre" : "Post",
			    eventName);
			return;
		}
