...
public:
	...
	void updateFromPlayerTitleData(const FPlayerTitleData& data)
	{
		...
		m_sameTextAndGlowColor =
			(m_textColor.R == m_glowColor.R) &&
			(m_textColor.G == m_glowColor.G) &&
			(m_textColor.B == m_glowColor.B) &&
...
private:
	static constexpr int MAX_TEXT_LENGTH = 64; // max characters that will show up in UI (excluding {brace} symbols)... anything more wont show up
	...
	std::unordered_map<UGFxData_PRI_TA*, TitleAppearance> m_ingameCustomPresets;	// <--- or APRI_TA* as the key if that works better, with the chat data nd all that

	bool m_shouldOverwriteGetTitleDataReturnVal = false; // what a name
	std::wstring m_selectedTitleId;
